...
TODO change value "no" in variable "tracking" to "no tracking"
...
levels_tracking = c("no t racking", "free", "paid")

#?as.factor

survey_factor <- survey_date |>
    mutate(run_loc = factor(run_loc, levels = levels_loc)) |>
    mutate(tracking = as.factor(tracking))
```
...
geom_bar() +
labs(title = "Where do you run and how track your runs")
```
